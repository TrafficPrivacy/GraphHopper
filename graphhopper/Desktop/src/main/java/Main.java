...
public class Main {
    ...
    */
    ...
    public static void Server(){
        ...
        Double inputQueryLat = 0.0;
        Double inputQueryLon = 0.0;

        Double inputQueryLat2 = 0.0;
        Double inputQueryLon2 = 0.0;
        ...
        try {
            ...
            while(true) {
                try {
                    ...
                    inputQueryLat = in.readDouble();
                    inputQueryLon = in.readDouble();

                    inputQueryLat2 = in.readDouble();
                    inputQueryLon2 = in.readDouble();

                    System.out.println(inputQueryLat + ", " + inputQueryLon + ", " + inputQueryLat2 + ", " + inputQueryLon2);
                    ...
                    //ALGORITHM START
                    //////////////////////////////////////////////
                    double queryLat = 40.743033467535376;
                    double queryLon = -73.9800787903396;

                    queryLat = inputQueryLat;
                    queryLon = inputQueryLon;

                    double queryLat2 = 40.734017992740625;
                    double queryLon2 = -73.98982397776714;

                    queryLat2 = inputQueryLat2;
                    queryLon2 = inputQueryLon2;
                    ...
                    BFSTest.AdjMatrixReturn adjmatrix_return = engine.BFSAdjMatrix(queryLat, queryLon, edgeFilter, encoder);
        ...
        for(int i = 0; i < fw_matrix.length; i++){
            ...
            System.out.println("\n");
                    ...
                    BFSTest.AdjMatrixReturn adjmatrix_return2 = engine.BFSAdjMatrix(queryLat2, queryLon2, edgeFilter, encoder);
        ...
        */
                    ...
                    try{
                        ...
                        gzip.write(total_json.toString().getBytes("UTF-8"));
                        gzip.close();
                    ...
                    int max = 1000;
                    int x = compressed.length / max;
                    int r = (compressed.length % max); // remainder
                    ...
                    for(int i=0; i<x; i++){
                        ...
                        out.writeInt(max);
                        out.write(Arrays.copyOfRange(compressed, lower, upper));
                    ...
                    if(r > 0){
                        ...
                        out.writeInt(r);
                        out.write(Arrays.copyOfRange(compressed, lower, (lower + r)));
                    ...
                    try {
                        BufferedWriter outfile = new BufferedWriter(new FileWriter("/Users/Edward_Chou/Documents/json_file.txt"));
                        outfile.write(total_json.toString());  //Replace with the string
                        //you are trying to write
                        outfile.close();

                        //final byte[] convertBytes= total_json.toString().getBytes();
                        //System.out.println("String length: " + convertBytes.length);
                    }
                    catch (IOException e)
                    {
                    ...
                    int source_idx = circum_points.size()/2;
                    int end_idx = circum_points2.size()/2;
    ...
    private static void Client(){
        ...
        String csvFile = "/Users/Edward_Chou/Downloads/yellow_tripdata_2016-01.csv";
        ...
        try {
            ...
            int csv_counter = 0;
            csvline = br.readLine();
            while ((csvline = br.readLine()) != null && csv_counter < 5) {
                ...
                String[] array = csvline.split(cvsSplitBy);
                Double start_lon = Double.parseDouble(array[5]);
                Double start_lat = Double.parseDouble(array[6]);

                Double end_lon = Double.parseDouble(array[9]);
                Double end_lat = Double.parseDouble(array[10]);

                if(start_lon == 0 && start_lat == 0 && end_lon == 0 && end_lat == 0){
                    continue;
                }

                long protocoltimestart = System.currentTimeMillis();

                out.writeDouble(start_lat);
                out.writeDouble(start_lon);

                out.writeDouble(end_lat);
                out.writeDouble(end_lon);


                InputStream inFromServer = client.getInputStream();
                DataInputStream in = new DataInputStream(inFromServer);

                Integer compressed_length = in.readInt();

                byte[] compressed = new byte[0];

                while(compressed_length > 0) {
                    //System.out.println("Compressed length: " + compressed_length);

                    byte[] sub_compressed = new byte[compressed_length];
                    in.read(sub_compressed);
                    byte[] new_compressed = new byte[compressed.length + sub_compressed.length];
                    System.arraycopy(compressed, 0, new_compressed, 0, compressed.length);
                    System.arraycopy(sub_compressed, 0, new_compressed, compressed.length, sub_compressed.length);
                    compressed = new_compressed;

                    compressed_length = in.readInt();
                ...
                try {
                    ...
                    final byte[] convertBytes= outStr.getBytes();
                    System.out.println("String length: " + convertBytes.length + ", compressed length: " + compressed.length);

                    JSONObject total_json = new JSONObject(outStr);
                    ...
                    for(int i = 0; i < circum_dist_json_array.length(); i++) {
                        ...
                        for (int j = 0; j < circum_dist_json_row.length(); j++) {
                            ...
                            try {
                                ...
                                Integer int_elem = (Integer) circum_dist_json_row.get(j);
                                Double elem = (double) int_elem;
                                circum_dist[i][j] = elem;
                            }
                    ...
                    for(int i = 0; i < fw_matrix_json_array.length(); i++) {
                        ...
                        for (int j = 0; j < fw_matrix_json_row.length(); j++) {
                            try {
                                ...
                                Integer int_elem = (Integer) fw_matrix_json_row.get(j);
                                Double elem = (double) int_elem;
                                fw_matrix[i][j] = elem;
                            }
                    ...
                    for(int i = 0; i < fw_matrix2_json_array.length(); i++) {
                        ...
                        for (int j = 0; j < fw_matrix2_json_row.length(); j++) {
                            ...
                            try {
                                ...
                                Integer int_elem = (Integer) fw_matrix2_json_row.get(j);
                                Double elem = (double) int_elem;
                                fw_matrix2[i][j] = elem;
                            }
                        }
...
//////////
                    ...
                    //System.out.println(circum_index2 + ", " + end_idx);
                    ...
                    long protocoltimeend = System.currentTimeMillis();

                    long protocoltime = protocoltimeend - protocoltimestart;
                    System.out.println("protocol time: " + protocoltime);
